...
# 计算LOD值
LOD = log10(
  dbinom(merged_data$alt_reads_mut, 
         size = merged_data$alt_reads_mut + merged_data$ref_reads_mut, 
         prob = mut_freq) *
    dbinom(merged_data$alt_reads_wt, 
           size = merged_data$alt_reads_wt + merged_data$ref_reads_wt, 
           prob = wt_freq) /
    (dbinom(merged_data$alt_reads_mut, 
            size = merged_data$alt_reads_mut + merged_data$ref_reads_mut, 
            prob = (mut_freq + wt_freq) / 2) *
       dbinom(merged_data$alt_reads_wt, 
              size = merged_data$alt_reads_wt + merged_data$ref_reads_wt, 
              prob = (mut_freq + wt_freq) / 2))
)
...
```{r}
# 合并图像
jpeg("LOD_all_chromosomes.jpg", width = 2100*2, height = 2970*2, res = 300)  # A4大小，300 dpi
  
  # 设置 6 行 5 列的画板
  par(mfrow = c(6, 5), mar = c(4, 4, 2, 1))  # mar 控制边距，避免图像太紧
